...
<div style="float: left; clear: both; margin-top: 30px; margin-bottom: 30px; margin-left: 24%;">
  <textarea rows="6" cols="100%" name="descripcion" placeholder="Cuentanos de tu Mascota"></textarea>
</div>

<div style="float: left; clear: both; margin-left: 45%">
   <select name="adopcion" style="margin-bottom: 10%;">
         <option value="-1" selected disabled>Es una Mascota en adopcion??</option>
...
if (isset($_POST['submit'])) {  
  ...
  if ($imagen_tipo == IMAGETYPE_PNG OR $imagen_tipo == IMAGETYPE_JPEG OR $imagen_tipo == IMAGETYPE_BMP OR IMAGETYPE_GIF) {

  $filtro = $mysqli->real_escape_string($_POST['filter']);
  $descripcion = $mysqli->real_escape_string($_POST['descripcion']);
  $adopcion = $mysqli->real_escape_string($_POST['adopcion']);
    ...
    if(is_uploaded_file($_FILES['file-input']['tmp_name'])) { 
        ...
        if($_FILES['file-input']['tmp_name']) {
          ...
          $queryp = $mysqli->query("INSERT INTO publicaciones (user,descripcion,fecha,adopcion) VALUES ('".$_SESSION['id']."','".$descripcion."',now(),'".$adopcion."')");
          echo "$query";
